...
*   **Bắt buộc** sử dụng mô hình Strongly-typed Configuration: Định nghĩa class cấu hình tương ứng với json settings, kiểm chứng bằng Data Annotations (`[Required]`, `[MinLength]`), và inject thông qua **`IOptions<T>`** hoặc **`IOptionsSnapshot<T>`** (khi cần reload động cấu hình cấu trúc settings).
...
## 🌐 3. Quy Tắc Cho Frontend (React TypeScript - cái-tiệm-hoa-của-chin)

### A. Tiêu Chuẩn React & TypeScript
*   **Tuyệt đối không dùng `any`**. Mọi biến, props và response đều phải được định nghĩa interface/type rõ ràng.
*   Sử dụng **Functional Components** kết hợp cùng **React Hooks**. Các logic phức tạp, fetch data phải được tách riêng thành **Custom Hooks** (ví dụ: `useProductSearch`).
*   **Tên file & Thư mục:**
    *   File React components sử dụng tên dạng `PascalCase.tsx` (ví dụ: `ProductCard.tsx`).
    *   File logic / helper / hooks sử dụng tên dạng `camelCase.ts` hoặc `kebab-case.ts` (ví dụ: `useProductSearch.ts`, `api-client.ts`).

### B. UI/UX Cao Cấp (Premium Aesthetic & UI-UX Pro Max)
*   **Aesthetic First:** Giao diện phải đẹp mắt, tạo ấn tượng cao cấp. Sử dụng các phong cách thiết kế hiện đại như **Minimalism (Tối giản hiện đại)** hoặc **Glassmorphism (Hiệu ứng kính mờ)**.
*   **Typography:** Sử dụng các font chữ cao cấp từ Google Fonts (như `Inter`, `Outfit`, `Playfair Display`) với tỷ lệ kích thước chữ (scale) hài hòa.
*   **Animations:** Thêm subtle micro-animations (hiệu ứng hover nhẹ, slide/fade in mượt mà) cho các phần tử nút bấm, thẻ thông tin để tạo cảm giác giao diện "sống động".
*   **Responsive:** Bố cục dạng Bento Grid hoặc Flexbox linh hoạt, tương thích hoàn hảo trên di động, máy tính bảng và màn hình lớn. Không để vỡ layout hoặc tràn màn hình.
...
## 🛡️ 6. Quy Tắc Bảo Mật & Xác Thực (Security & Authentication)
...
*   **Unit Tests:** Mọi logic nghiệp vụ trong Application layer phải có Unit Test bao phủ đầy đủ cả kịch bản thành công và thất bại.
